...
// Adds the enemy to the grid at the specified coordinates
- (void)addEnemyToCoordinateWithX:(float)x Y:(float)y
{
    // Create the enemy to add
    [self createJellyfishAtX:(float)x Y:(float)y];
    
    float below = 75;
    
    // Set the coordinates and add them to the scene
    ...
    coordinates.position = CGPointMake(_jellyfish.position.x, _jellyfish.position.y - below);
...
- (void)attackCoordinateWithX:(float)x Y:(float)y
{
    ...
    //[self createMoveToTargetAtX:x Y:y ForDuration:2];
