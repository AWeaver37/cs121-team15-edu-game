...
-(id)initWithSize:(CGSize)size {    
    if (self = [super initWithSize:size]) {
        ...
        CGPathAddLineToPoint(xPath, NULL, self.axisLength, 0);
        xAxis.path = xPath;
        ...
        CGPathAddLineToPoint(yPath, NULL, 0, self.axisLength);
        yAxis.path = yPath;
        ...
        float offset = 50;
        float realTickMarkSpacing = (tickMarkSpacing/axisGameLength)*self.axisLength;
        
        //x-axis
        float realX = self.origin.x;
        float gameX = 0;
        for (; realX <= self.origin.x + self.axisLength; realX += realTickMarkSpacing, gameX += tickMarkSpacing)
        {
            ...
            if (gameX != 0) {
                ...
                [tick setStrokeColor:[UIColor blueColor]];
                [self addChild:tick];
            }
        ...
        float gameY = 0;
        for (; realY <= self.origin.y + self.axisLength; realY += realTickMarkSpacing, gameY += tickMarkSpacing)
        {
